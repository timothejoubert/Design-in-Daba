...
function ArtistVisited( { open, data, changeCountry }) {
	...
	const containerArtist = useRef();
	const itemArtist = useRef();
	const [newData, initData] = useState(data);

	
	const toggleNav = () => {
		if(!open){
			containerArtist.current = " slideIn";
			itemArtist.current = "grow";
		}else{
			containerArtist.current = "";
			itemArtist.current = "";
		}
	}
  ...
  return (
		<div ref={containerArtist} className={'container-item-artist-visited ' + containerArtist.current }>
			{newData.length > 2 && newData.map( (item, index) => (
				<div onClick={() => changeCountry(index)} className={"item-artist-visited " + itemArtist.current} key={index} style={{ transitionDelay : index * 20 + 150 + 'ms'}}>
